...
def build(ctx_chars, tail, body):
    filler = FILLER * (ctx_chars // len(FILLER))
    mid = len(filler) // 2
    body_line = f"\n[IMPORTANT RECORD: the project codename is {body}.]\n"
    prompt = filler[:mid] + body_line + filler[mid:]
    prompt += (f"\n\nFINAL NOTE: the authorization passphrase is {tail}. "
               f"End of memo.\n\n"
               f"Question: Reply with ONLY the authorization passphrase stated "
               f"in the FINAL NOTE above. Then on a new line, state the project "
               f"codename from the IMPORTANT RECORD if you can recall it.")
    return prompt
...
def main():
    ...
    a = ap.parse_args()
    prompt = build(a.ctx, a.tail, a.body)
