...
struct GemmaModel {
    ...
    void init_rope(int max_seq) {
        ...
        rope_slide.init(max_seq, cfg.slide_hd, cfg.slide_rope_dim, cfg.slide_rope_theta, ng);
        // Full: theta=1M, rope_dim=128, partial rotation of 512-dim heads
        rope_full.init(max_seq, cfg.full_hd, cfg.full_rope_dim, cfg.full_rope_theta, ng);
    }
    ...
    void forward_full_attn(int layer, half* hidden, int pos, cudaStream_t stream) {
        ...
        // 7. Standard attention scoring (FP16 KV cache)
        float scale = 1.0f / sqrtf((float)hd);
        dim3 grid_score(num_q, seq_len);
        // 256 threads ⇒ proper cross-warp reduction inside attn_score_kernel_h
        attn_score_kernel_h<<<grid_score, std::min(hd, 256), 0, stream>>>(
            b.q_proj, k_cache_ptr, b.attn_scores,
